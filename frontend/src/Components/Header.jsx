...
function Header() {
  ...
  const handleRestrictedAction = (e, destination) => {
    ...
    if (isReturningUser === "true") {
      setToastMessage(
        "Welcome back, Chief! Please login to view this content.",
      );
      setShowToast(true);
      setTimeout(() => {
        setShowToast(false);
      ...
      setToastMessage("Please verify your account to unlock full access!");
      setShowToast(true);
      setTimeout(() => {
        setShowToast(false);
    ...
    <div className="fixed top-0 left-0 right-0 flex flex-col items-center z-[100] px-4 py-6 pointer-events-none">
      <AnimatePresence>
        {showToast && (
          ...
          >
            <div
              className={`p-2 rounded-full ${user ? "bg-green-100 text-green-600" : "bg-orange-100 text-orange-600"}`}
            >
              {user ? <FiCheckCircle size={20} /> : <FiLock size={20} />}
            </div>
            <div>
              <p className="text-[10px] font-black uppercase text-gray-800 tracking-tight">
                KKB Mastery
              </p>
      ...
      >
        <Link to="/" className="flex items-center gap-3 shrink-0">
          ...
          <div className="flex flex-col -space-y-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-gray-300 ml-1 italic">
              Mastery
            </span>
          </div>
        </Link>
        ...
        >
          ...
          >
            ...
            <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-3xl shadow-2xl border border-gray-100 p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
              <div className="flex flex-col gap-2">
                <DropdownItem
                  onClick={(e) =>
                    handleRestrictedAction(e, "/discover?cat=junk")
                  }
                  to="/discover?cat=junk"
                  icon={<FiSmile />}
                  title="Junk"
                  subtitle="Quick Treats"
                />
                <DropdownItem
                  onClick={(e) =>
                    handleRestrictedAction(e, "/discover?cat=breakfast")
                  }
                  to="/discover?cat=breakfast"
                  icon={<FiCoffee />}
                  title="Breakfast"
                ...
                <DropdownItem
                  onClick={(e) =>
                    handleRestrictedAction(e, "/discover?cat=dinner")
                  }
                  to="/discover?cat=dinner"
                  icon={<FiZap />}
                  title="Dinner"
                  subtitle="Night Specials"
                />
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-3xl shadow-2xl border border-gray-100 p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
              <div className="flex flex-col gap-2">
                <DropdownItem
                  onClick={(e) => handleRestrictedAction(e, "/create")}
                  to="/create"
                  icon={<FiPlusCircle />}
                  title="Create"
                  subtitle="New magic"
                />
                <DropdownItem
                  onClick={(e) => handleRestrictedAction(e, "/favorites")}
                  to="/favorites"
                  icon={<FiHeart />}
                  title="Favourite"
                  subtitle="Your Loves"
                />
                <DropdownItem
                  onClick={(e) => handleRestrictedAction(e, "/planner")}
                  to="/planner"
                  icon={<FiBarChart2 />}
                  title="Meal Planner"
                  subtitle="Schedule"
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          {user ? (
            <div className="relative" ref={profileRef}>
              ...
              >
                ...
                <div className="text-left hidden md:block">
                  ...
                  <p className="text-[8px] font-bold text-orange-500 uppercase tracking-tighter">
                    {user.role}
                  </p>
                </div>
              </motion.button>
                  ...
                  >
                    ...
                    {user.role === "admin" && (
                      <DropdownItem
                        ...
                        icon={<FiLayout />}
                        title="Admin Panel"
                        subtitle={`Manage (${pendingCount})`}
                      />
                    )}
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-4 w-full p-4 hover:bg-red-50 rounded-[1.8rem] transition-all text-left group"
                    >
            ...
            <div className="flex items-center gap-2">
              <Link
                to="/login"
                className="text-[10px] font-black uppercase text-gray-400 px-4"
              >
                Login
              </Link>
              <Link to="/register">
                <motion.button
                  ...
                  className="bg-orange-600 text-white px-8 py-3 rounded-full font-black text-[11px] uppercase tracking-widest shadow-lg"
                >
                  Join
                </motion.button>
              </Link>
              ...
              <div className="w-full max-w-5xl bg-white rounded-[3.5rem] shadow-2xl border border-orange-50 overflow-hidden">
                <div className="p-8 flex items-center border-b border-gray-50 bg-gray-50/20 gap-4">
                  ...
                  <input
                    ...
                    className="w-full h-14 outline-none font-bold text-gray-800 bg-transparent text-xl"
                  />
                  <button
                    onClick={handleVoiceSearch}
                    className="p-3 bg-orange-50 text-orange-600 rounded-full hover:bg-orange-600 hover:text-white transition-all"
                  >
                    <FiMic size={20} />
                  </button>
                  <FiX
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-8 max-h-[500px] overflow-y-auto">
                  ...
                  <div className="space-y-8 border-l border-gray-50 pl-6">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button
                        onClick={clearRecent}
                        className="text-[8px] font-bold text-orange-600 uppercase underline"
                      >
                        Clear
                      </button>
                    </div>
...
const DropdownItem = ({ to, icon, title, subtitle, onClick }) => (
  <Link
    to={to}
    onClick={onClick}
    className="flex items-center gap-4 p-4 hover:bg-orange-50 rounded-[2rem] transition-all group/item"
  >
    ...
    <div className="flex flex-col text-left">
      <span className="text-[10px] font-black uppercase text-gray-800 leading-tight">
        {title}
      </span>
      <span className="text-[8px] font-bold text-gray-400 uppercase">
        {subtitle}
