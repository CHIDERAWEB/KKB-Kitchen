...
const RecipeGrid = ({ user }) => {
  ...
  const handleProtectedNavigation = (recipe, status) => {
    ...
    if (!rawId) {
      console.error("No valid ID found:", recipe);
      return;
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-40">
        ...
        <div className="font-bold text-orange-500 uppercase tracking-widest animate-pulse text-sm">
          Gathering Ingredients...
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 py-14">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-10 gap-4">
        <div>
          ...
          <div className="h-1.5 w-24 bg-orange-500 rounded-full mt-2" />
        </div>

        <p className="text-gray-400 font-semibold uppercase text-xs tracking-widest">
      ...
      <div className="grid grid-cols-2 space-x-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {recipes.length > 0 ? (
          recipes.map((recipe, index) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className={`group bg-white rounded-3xl shadow-md hover:shadow-xl transition-all duration-500 overflow-hidden border border-gray-100 flex flex-col relative ${isPending ? "cursor-default" : "cursor-pointer"}`}
                onClick={() =>
              ...
              >
                ...
                <div className="relative h-60 md:h-64 overflow-hidden">
                  <img
                    ...
                    alt={recipe.title || recipe.name}
                    className={`w-full h-full object-cover transition-transform duration-700 ${!isPending ? "group-hover:scale-105" : "blur-[2px] opacity-60"}`}
                  />

                  {/* rating */}
                  {!isPending && (
                    <div className="absolute top-4 left-4 bg-orange-500 text-white px-3 py-1 rounded-xl shadow flex items-center gap-1 text-xs font-bold">
                  ...
                  {isPending && (
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] flex items-center justify-center flex-col gap-2">
                      <div className="bg-white p-3 rounded-full shadow animate-bounce">
                        <FiLock className="text-orange-600" size={22} />
                      </div>
                      <span className="bg-orange-600 text-white text-[10px] font-bold uppercase tracking-widest px-4 py-1 rounded-full">
                        Under Review
                      </span>
                    </div>
                  )}
                  ...
                  {/* time */}
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-xl shadow flex items-center gap-2 text-xs font-semibold">
                    <FiClock className="text-orange-500" />
                    {recipe.cookTime || "30m"}
                  </div>
                </div>
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <h3
                    className={`text-lg font-bold leading-snug mb-5 transition-colors line-clamp-2 ${!isPending ? "text-gray-900 group-hover:text-orange-600" : "text-gray-400"}`}
                  >
                    {recipe.title || recipe.name}
                  </h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                    {/* chef */}
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-orange-100 flex items-center justify-center text-orange-600 border border-orange-200 overflow-hidden transition group-hover:rotate-3">
                        {recipe.author?.picture ? (
                          <img
                    ...
                    <button
                      ...
                      className="h-11 w-11 bg-gray-900 rounded-xl flex items-center justify-center text-white hover:bg-orange-600 transition active:scale-95"
                    >
                      {isPending ? (
                        <FiEye size={18} className="opacity-50" />
                      ) : (
          ...
          <div className="col-span-full text-center py-28 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
