    ...
    <div className="flex items-center">
      ...
      <div className="hidden lg:flex items-center gap-8 font-sans">
        ...
        <div className="relative" ref={discoverRef}>
          ...
          <AnimatePresence>
            {discoverOpen && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute left-0 mt-4 w-48 bg-white rounded-2xl shadow-2xl z-110 border border-gray-100 p-2 overflow-hidden">
                {mealTypes.map((item) => (
        ...
        <div className="relative" ref={kitchenRef}>
          ...
          <AnimatePresence>
            {kitchenOpen && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute left-0 mt-4 w-56 bg-white rounded-2xl shadow-2xl z-110 border border-gray-100 p-2">
        ...
        {user ? (
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={userRef}>
              ...
              <AnimatePresence>
                {userMenuOpen && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-52 bg-white rounded-2xl shadow-2xl z-110 border border-gray-100 p-2 overflow-hidden">
                    ...
                    {user?.role === 'admin' && (
                      <button onClick={() => handleLinkClick('/admin')} className="flex items-center justify-between w-full px-4 py-3 text-sm font-black text-purple-600 hover:bg-purple-50 rounded-xl transition-all">
                        ...
                        {pendingCount > 0 && (
                          <span className="bg-red-600 text-white text-[10px] px-2 py-0.5 rounded-full">{pendingCount} NEW</span>
                        )}
                      </button>
                    )}
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            <div className="flex flex-col gap-6 overflow-y-auto">
              ...
              {user?.role === 'admin' && (
                <Link to="/admin" onClick={() => setMobileMenuOpen(false)} className="text-2xl font-black text-purple-600 flex items-center justify-between">
              ...
              <Link to="/" onClick={() => setMobileMenuOpen(false)} className="text-2xl font-black text-gray-900">Home</Link>
              <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="text-2xl font-black text-gray-900 flex items-center gap-3">
                About <FiInfo className="text-orange-500" size={20} />
              </Link>

              <div className="h-px w-full bg-gray-100 my-2" />

              <Link to="/favorites" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold text-gray-600">My Favorites</Link>
              <Link to="/planner" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold text-gray-600">Meal Planner</Link>
              <Link to="/shopping-list" onClick={() => setMobileMenuOpen(false)} className="text-lg font-bold text-gray-600">Shopping List</Link>
