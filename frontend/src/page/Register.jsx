...
import { Check, ChefHat, Lock, Mail, User, X } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import { FiEye, FiEyeOff } from 'react-icons/fi';
import { Link, useNavigate } from 'react-router-dom';
// IMPORT YOUR VERIFY PAGE HERE
...
const Register = ({ triggerLoading, showToast }) => {
  ...
  useEffect(() => {
    ...
    // CASE 1: They verified already but didn't login? 
    // (In your VerifyEmail, you navigate to /login, but just in case they come back here)
    if (storedUser && !tempEmail) {
      navigate("/login");
      return;
    }

    // CASE 2: They registered but haven't verified (temp_email exists)
    if (tempEmail && !storedUser) {
      setIsVerifying(true);
      showToast("Welcome back! Please verify your account. 🍳");
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isFormValid || strengthScore < 4) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(`${formData.name}, check your email for the code! 🍳`);
        ...
        // IF EMAIL EXISTS: They might just need to verify!
        if (data.message?.toLowerCase().includes("already exists")) {
          showToast("Email already registered. Redirecting to verify... 🏃‍♂️");
          localStorage.setItem("temp_email", payload.email);
  ...
  return (
    <div className="min-h-screen flex bg-white overflow-hidden">
      {/* LEFT SIDE: DECORATIVE (Untouched) */}
              ...
              <>
                ...
                <form className="mt-10 space-y-5" onSubmit={handleSubmit}>
                  ...
                  <motion.button
                    ...
                    className={`w-full flex justify-center items-center gap-3 py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl transition-all mt-8 ${!isFormValid ? "bg-gray-400 text-gray-700 cursor-not-allowed" : "bg-orange-500 hover:bg-orange-600 text-white cursor-pointer"}`}
                  >
                    {isLoading ? (
                ...
                <div className="mt-8">
                  ...
                  <GoogleLogin
                    onSuccess={async (credentialResponse) => {
                      // ... (Your Google Login Logic remains exactly the same)
                      showToast(`Welcome! 🍳`);
                      navigate("/verify-email"); // Or handle Google logic similarly
                    }}
                ...
                <div className="mt-8 text-center">
                  <p className="text-sm text-gray-500 font-medium">
                    Already part of the kitchen? <Link to="/login" className="text-orange-600 font-black hover:underline ml-1">LOG IN</Link>
                  </p>
                </div>
              </>
            ) : (
              /* --- SHOW YOUR OTP COMPONENT --- */
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <VerifyEmail showToast={showToast} />
                <button
                  onClick={() => setIsVerifying(false)}
                  className="w-full mt-6 text-xs font-black text-gray-400 uppercase hover:text-orange-500"
                >
